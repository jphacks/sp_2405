...
- 学習者マッチング機能
  ホーム画面では、現在稼働している入室可能なポモドーロ部屋に入ることができます。部屋にはそれぞれタグを設定することができ、似たタスク同士の人がつながれるようにしています。
  ユーザーは他のユーザーが作成した部屋に入ることで、同時にポモドーロテクニックを実践します。もちろん、部屋はどのユーザーでも作成することができます。<br>
![](./home.png)
![](./create_room.png)
- ポモドーロタイマーによる時間管理
  ポモドーロ法の根幹である「25分間の作業」と「5分間の休憩」を強調するため、ページ画面にはそれぞれのタームの残り時間を表示します。単純にタイマをセットするのではなく、ポモドーロ部屋の開始時刻と現在時刻をセッションごとに比較することにより、より正確なタイマ機能を実現しています。作業時間中はシンプルなタイマーのみを表示することで集中を促し、他の画面要素は一切見せない工夫をしています。<br>
![](./study.png)
- 休憩時間中のリアルタイムな進捗共有
...
- 詳細な学習記録機能
  分析ページでは、「今日の取り組み」と「これまでの取り組み」が表示されます。それぞれ、総時間とサイクル数をグラフにより可視化しています。さらに「今月の成果」ということで1か月の間、どの時間にどれだけ進捗だ進んだかを集中度として、草形式のグラフで可視化しています。また分析ページには「取り組んだ内容」が表示されます。「取り組んだ内容」には、各サイクルの休憩時間で入力した「振り返りコメント」を表示しています。「25分間の作業」を行った日時、どれだけの進捗を生み（どれだけ集中したか）、どのような内容を行ったかを一目で確認することができます。<BR>
![](./analysis.png)
